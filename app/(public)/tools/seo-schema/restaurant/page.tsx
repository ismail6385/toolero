...
export default function RestaurantSchema() {
    const [formData, setFormData] = useState({
        name: '', description: '', url: '', telephone: '', priceRange: '', servesCuisine: '',
        addressStreet: '', addressLocality: '', addressRegion: '', addressPostalCode: '', addressCountry: '',
        openingHoursMonday: '', openingHoursTuesday: '', openingHoursWednesday: '', openingHoursThursday: '',
    ...
    const generateSchema = () => {
        const schema: any = {
            ...
            "servesCuisine": formData.servesCuisine || undefined,
        };
        ...
        dayFields.forEach((field, index) => {
            if (formData[field as keyof typeof formData]) {
                const hours = formData[field as keyof typeof formData] as string;
                openingHours.push(`${days[index]} ${hours}`);
            }
        });
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-12">
            <div className="mb-8 text-center">
                ...
                <h1 className="text-3xl md:text-4xl font-semibold text-text mb-2">Schema Restaurant</h1>
                <p className="text-text/60 max-w-2xl mx-auto">Genera Schema.org para restaurantes. Mejora la visibilidad en Google My Business.</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-surface rounded-xl shadow-md border border-gray-200 p-6">
                    <h2 className="text-xl font-semibold text-text mb-6 flex items-center gap-2">
                        <FontAwesomeIcon icon={faUtensils} className="text-primary" /> Información del Restaurante
                    </h2>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-semibold text-text mb-2">Nombre del Restaurante *</label>
                            <input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary focus:border-primary transition-colors" placeholder="Nombre del restaurante" />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-text mb-2">Descripción</label>
                            <textarea name="description" value={formData.description} onChange={handleChange} rows={3} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary focus:border-primary transition-colors resize-none" placeholder="Descripción del restaurante..." />
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-text mb-2">Tipo de Cocina</label>
                            <input type="text" name="servesCuisine" value={formData.servesCuisine} onChange={handleChange} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary focus:border-primary transition-colors" placeholder="Italiana, Española, etc." />
                        </div>
                            ...
                            <div className="space-y-3">
                                {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day, index) => (
                                    <div key={day}>
                                        ...
                                        <input type="text" name={`openingHours${day}`} value={formData[`openingHours${day}` as keyof typeof formData] as string} onChange={handleChange} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary focus:border-primary transition-colors" placeholder="09:00-22:00" />
                                    </div>
                                ))}
                            </div>
                        </div>
