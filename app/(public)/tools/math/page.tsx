...
export default function MathToolsPage() {
    const category = toolsData.find(c => c.slug === 'math');
    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            <section className="relative w-full overflow-hidden bg-surface pb-16 pt-24 lg:pt-32">
                <div className="absolute -top-24 -left-20 w-96 h-96 bg-green-500/10 rounded-full blur-3xl mix-blend-multiply opacity-50" />
                ...
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
                    ...
                    <nav className="mb-8 text-sm" aria-label="Breadcrumb">
                        <ol className="flex items-center justify-center space-x-2 text-text/60">
                            ...
                            <li><Link href="/categorias" className="hover:text-green-600 transition-colors">Categorías</Link></li>
                            <li><FontAwesomeIcon icon={faArrowRight} className="text-xs mx-2" /></li>
                            <li>
                                <span className="text-green-600 font-semibold" aria-current="page">
                                    {category.name}
                                </span>
                            </li>
                        </ol>
                    ...
                    <div className="text-center mb-12">
                        <div className="inline-flex items-center px-3 py-1 rounded-full border border-green-500/20 bg-green-500/5 text-green-600 text-xs font-semibold uppercase tracking-wide mb-6">
                            <FontAwesomeIcon icon={category.icon} className="mr-2" />
                            {category.name}
                        </div>
                        <h1 className="text-4xl md:text-6xl font-semibold text-text tracking-tight mb-6">
                            Herramientas de <span className="text-green-600">Matemáticas</span>
                        </h1>
                        <p className="mt-4 text-lg md:text-xl text-text/60 max-w-3xl mx-auto leading-relaxed">
                            {category.description} Cálculos rápidos, precisos y gratuitos.
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-3 gap-4 max-w-2xl mx-auto">
                        <div className="bg-background rounded-xl p-4 text-center border border-gray-100">
                            <div className="text-2xl font-bold text-green-600 mb-1">{category.tools.length}</div>
                            <div className="text-xs text-text/60 font-semibold">Herramientas</div>
                        </div>
                        <div className="bg-background rounded-xl p-4 text-center border border-gray-100">
                            <div className="text-2xl font-bold text-green-600 mb-1">100%</div>
                            <div className="text-xs text-text/60 font-semibold">Preciso</div>
                        </div>
                        <div className="bg-background rounded-xl p-4 text-center border border-gray-100">
                            <div className="text-2xl font-bold text-green-600 mb-1">∞</div>
                            <div className="text-xs text-text/60 font-semibold">Uso Libre</div>
                        </div>
                    </div>
                </div>
            </section>
